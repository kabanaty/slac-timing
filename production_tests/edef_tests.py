...
class EdefReservationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.initial_edefs_available = epics.caget(
            "IOC:{iocloc}:EV01:EDEFAVAIL".format(iocloc=self.ioc_location)
        )
    ...
    def test_available_count_drops(self):
        edefs_available_after_setup = epics.caget(
            "IOC:{iocloc}:EV01:EDEFAVAIL".format(iocloc=self.ioc_location)
        )
        self.assertTrue(edefs_available_after_setup < self.initial_edefs_available)

    def test_edef_name_is_correct(self):
        name_pv = "EDEF:{sys}:{num}:NAME".format(
            sys=self.edef.sys, num=self.edef.edef_num
        )
...
class ExistingEdefTest(unittest.TestCase):
    def setUp(self):
        ...
        self.name = "edef.py unit tests {}".format(randint(0, 255))
        epics.caput(
            "IOC:{iocloc}:EV01:EDEFNAME".format(iocloc=self.ioc_location), self.name
        )
        ...
        if self.edef_num is None:
            raise RuntimeError(
                "Manual edef reservation failed, cannot proceed with test."
            )
    ...
    def test_existing_configuration_not_overwritten(self):
        ...
        num_avg = 13
        epics.caput(
            "EDEF:{sys}:{num}:AVGCNT".format(sys=self.sys, num=self.edef_num), num_avg
        )
        num_meas = 20
        epics.caput(
            "EDEF:{sys}:{num}:MEASCNT".format(sys=self.sys, num=self.edef_num), num_meas
        )
        edef_obj = edef.EventDefinition(
            "should ignore", edef_number=self.edef_num, avg=1, measurements=1
        )
        self.assertEqual(edef_obj.n_avg, num_avg)
        self.assertEqual(edef_obj.n_measurements, num_meas)
        current_name = epics.caget(
            "EDEF:{sys}:{num}:NAME".format(sys=self.sys, num=self.edef_num)
        )
    ...
    def tearDown(self):
        ...
        if self.edef_num is not None:
            epics.caput(
                "EDEF:{sys}:{num}:FREE".format(sys=self.sys, num=self.edef_num), 1
            )
...
class EdefReleaseTest(unittest.TestCase):
    def setUp(self):
        ...
        if self.accelerator == "LCLS":
            self.ioc_location = "IN20"
        self.edef = edef.EventDefinition(
            "edef.py unit tests " + str(randint(0, 255)), os.uname()[1]
        )

    def test_release(self):
        edefs_available_before_release = epics.caget(
            "IOC:{iocloc}:EV01:EDEFAVAIL".format(iocloc=self.ioc_location)
        )
        ...
        self.edef.release()
        edefs_available_after_release = epics.caget(
            "IOC:{iocloc}:EV01:EDEFAVAIL".format(iocloc=self.edef.ioc_location)
        )
        self.assertEqual(
            edefs_available_before_release, edefs_available_after_release - 1
        )
...
class EdefPropertiesTest(unittest.TestCase):
    ...
    def test_inclusion_masks(self):
        ...
        mask_1 = epics.caget(
            "EDEF:{sys}:{num}:INCM1.DESC".format(
                sys=self.edef.sys, num=self.edef.edef_num
            )
        )
        mask_2 = epics.caget(
            "EDEF:{sys}:{num}:INCM2.DESC".format(
                sys=self.edef.sys, num=self.edef.edef_num
            )
        )
        masks = [mask_1, mask_2]
        self.edef.inclusion_masks = masks
        read_mask_1 = epics.caget(
            "EDEF:{sys}:{num}:INCM1".format(sys=self.edef.sys, num=self.edef.edef_num)
        )
        read_mask_2 = epics.caget(
            "EDEF:{sys}:{num}:INCM2".format(sys=self.edef.sys, num=self.edef.edef_num)
        )
    ...
    def test_exclusion_masks(self):
        ...
        read_mask_1 = epics.caget(
            "EDEF:{sys}:{num}:EXCM1".format(sys=self.edef.sys, num=self.edef.edef_num)
        )
        read_mask_2 = epics.caget(
            "EDEF:{sys}:{num}:EXCM2".format(sys=self.edef.sys, num=self.edef.edef_num)
        )
        self.assertEqual(read_mask_1, 1)
        self.assertEqual(read_mask_2, 1)
...
class AcquisitionTest(unittest.TestCase):
    def setUp(self):
        self.num_meas = 55
        self.edef = edef.EventDefinition(
            "edef.py unit tests " + str(randint(0, 255)),
            user=os.uname()[1],
            avg=1,
            measurements=self.num_meas,
        )
    ...
    def test_single_acquisition(self):
        if self.edef.sys != "SYS0":
            print(
                "Acquisition test only works on the LCLS network right now, skipping."
            )
            return
        self.edef.start()
        timeout = 25.0
