...
class EdefReservationTest(unittest.TestCase):
    ...
    def test_edef_name_is_correct(self):
        name_pv = "BSA:{sys}:1:{num}:NAME".format(
            sys=self.edef.sys, num=self.edef.number
        )
...
class ExistingEdefTest(unittest.TestCase):
    def setUp(self):
        ...
        if self.edef_num is None:
            raise RuntimeError(
                "Manual buffer reservation failed, cannot proceed with test."
            )
    ...
    def test_existing_configuration_not_overwritten(self):
        ...
        num_avg = 13
        epics.caput(
            "BSA:{sys}:1:{num}:AVGCNT".format(sys=self.sys, num=self.edef_num), num_avg
        )
        num_meas = 20
        epics.caput(
            "BSA:{sys}:1:{num}:MEASCNT".format(sys=self.sys, num=self.edef_num),
            num_meas,
        )
        time.sleep(1.0)
        edef_obj = edef.BSABuffer(
            "should ignore", number=self.edef_num, avg=1, measurements=1
        )
        time.sleep(1.0)
        self.assertEqual(edef_obj.n_avg, num_avg)
        self.assertEqual(edef_obj.n_measurements, num_meas)
        current_name = epics.caget(
            "BSA:{sys}:1:{num}:NAME".format(sys=self.sys, num=self.edef_num)
        )
    ...
    def tearDown(self):
        ...
        if self.edef_num is not None:
            epics.caput(
                "BSA:{sys}:1:{num}:FREE".format(sys=self.sys, num=self.edef_num), 1
            )


class EdefReleaseTest(unittest.TestCase):
    def setUp(self):
        self.edef = edef.BSABuffer(
            "sc_buffer.py unit tests " + str(randint(0, 255)), os.uname()[1]
        )
    ...
    def test_release(self):
        ...
        if not self.edef.is_reserved():
            raise RuntimeError(
                "BSABuffer could not be reserved, cannot proceed with test."
            )
...
class EdefPropertiesTest(unittest.TestCase):
    ...
    def test_destination_masks(self):
        ...
        mask_1 = epics.caget(
            "BSA:{sys}:1:{num}:DST0.DESC".format(
                sys=self.edef.sys, num=self.edef.number
            )
        )
        mask_2 = epics.caget(
            "BSA:{sys}:1:{num}:DST1.DESC".format(
                sys=self.edef.sys, num=self.edef.number
            )
        )
        masks = [mask_1, mask_2]
        ...
        read_mask_1 = epics.caget(
            "BSA:{sys}:1:{num}:DST0".format(sys=self.edef.sys, num=self.edef.number)
        )
        read_mask_2 = epics.caget(
            "BSA:{sys}:1:{num}:DST1".format(sys=self.edef.sys, num=self.edef.number)
        )
...
class AcquisitionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.edef = edef.BSABuffer(
            "sc_buffer.py unit tests " + str(randint(0, 255)),
            user=os.uname()[1],
            avg=1,
