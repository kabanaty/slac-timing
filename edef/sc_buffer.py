...
class BSABuffer(object):
    ...
    def __init__(
        ...
        destination_masks=None,
        avg_callback=None,
        measurements_callback=None,
        ctrl_callback=None,
        rate_mode=None,
        fixed_rate=None,
        ac_rate=None,
        ...
        if number is None:
            ...
            self.number = number
        self.sys = "SYS0"
        self.n_avg_pv = epics.PV(
            "{prefix}:{num}:AVGCNT".format(prefix=self.prefix, num=self.number)
        )
        self.n_measurements_pv = epics.PV(
            "{prefix}:{num}:MEASCNT".format(prefix=self.prefix, num=self.number)
        )
        self.ctrl_pv = epics.PV(
            "{prefix}:{num}:CTRL".format(prefix=self.prefix, num=self.number)
        )
        self.num_acquired_pv = epics.PV(
            "{prefix}:{num}:CNT".format(prefix=self.prefix, num=self.number)
        )
        self.rate_mode_pv = epics.PV(
            "{prefix}:{num}:RATEMODE".format(prefix=self.prefix, num=self.number)
        )
        self.destination_mode_pv = epics.PV(
            "{prefix}:{num}:DESTMODE".format(prefix=self.prefix, num=self.number)
        )
        self.destination_mask_pv = epics.PV(
            "{prefix}:{num}:DESTMASK".format(prefix=self.prefix, num=self.number)
        )
        self.fixed_rate_pv = epics.PV(
            "{prefix}:{num}:FIXEDRATE".format(prefix=self.prefix, num=self.number)
        )
        self.ac_rate_pv = epics.PV(
            "{prefix}:{num}:ACRATE".format(prefix=self.prefix, num=self.number)
        )
        self.timeslot_mask_pv = epics.PV(
            "{prefix}:{num}:TSLOTMASK".format(prefix=self.prefix, num=self.number)
        )
        self.measurement_severity_pv = epics.PV(
            "{prefix}:{num}:MEASSEVR".format(prefix=self.prefix, num=self.number)
        )
        self.data_ready_pv = epics.PV(
            "{prefix}:{num}:HST_READY".format(prefix=self.prefix, num=self.number)
        )
    ...
    def reserve(self, name, user=None):
        ...
        while time_elapsed < timeout:
            for num in range(21, 50):
                buffer_name = epics.caget(
                    "{prefix}:{num}:NAME".format(prefix=self.prefix, num=num)
                )
                if buffer_name == name:
                    if user is not None:
                        epics.caput(
                            "{prefix}:{num}:USERNAME".format(
                                prefix=self.prefix, num=num
                            ),
                            str(user),
                        )
                    return num
    ...
    def measurements_callback(self, new_callback):
        ...
        if new_callback is not None:
            ...
            self._measurements_callback_index = self.n_measurements_pv.add_callback(
                full_cb
            )
    ...
    def destination_masks(self, masks):
        ...
        if isinstance(masks, dict):
            for mask, val in masks.iteritems():
                ...
                destination_selection_pv = "{prefix}:{num}:DST{n}".format(
                    prefix=self.prefix, num=self.number, n=bit_num
                )
    ...
    def clear_masks(self):
        ...
        for i in range(0, 6):
            destination_selection_pv = "{prefix}:{num}:DST{n}".format(
                prefix=self.prefix, num=self.number, n=i
            )
    ...
    def populate_bit_mask_name_cache(self):
        ...
        bit_name_pvs = [
            "{prefix}:{num}:DST{n}.DESC".format(
                prefix=self.prefix, num=self.number, n=n
            )
            for n in bit_nums
        ]
        bit_names = epics.caget_many(bit_name_pvs)
        self.bit_mask_name_cache = {
            bit_name: bit_num for bit_name, bit_num in zip(bit_names, bit_nums)
        }
        self.bit_mask_reverse_cache = {
            bit_num: bit_name for bit_name, bit_num in zip(bit_names, bit_nums)
        }
    ...
    def start(self, callback=None):
        ...
        start = time.time()
        while (
            self.is_acquisition_complete()
        ):  # make sure the data ready flag switches before proceeding
            time.sleep(0.01)  # wait a small ammount of time
            if time.time() - start >= time_out:  # time out after a while
                raise Exception(
                    "BSA Buffer was not able to start, cannot acquire data."
                )
                return False
        return True
    ...
    def get_buffer(self, pv, suffix="HST"):
        ...
        if isinstance(pv, str):  # Single PV
            ...
            buff_dict = {
                a_pv[:-suffix_length]: buff_dict[a_pv][0:actual_acquired]
    ...
    def release(self):
        ...
        epics.caput(
            "{prefix}:{num}:FREE".format(prefix=self.prefix, num=self.number), 1
        )
