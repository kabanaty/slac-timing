...
class EventDefinition(object):
    def __init__(
        ...
        if edef_number is None:
            self.edef_num = self.reserve_edef(
                name, self.sys, self.accelerator, user=user
            )
            print("Reserved EDEF {}".format(self.edef_num))
        else:
            self.edef_num = edef_number
        self.n_avg_pv = epics.PV(
            "EDEF:{sys}:{num}:AVGCNT".format(sys=self.sys, num=self.edef_num)
        )
        self.beamcode_pv = epics.PV(
            "EDEF:{sys}:{num}:BEAMCODE".format(sys=self.sys, num=self.edef_num)
        )
        self.n_measurements_pv = epics.PV(
            "EDEF:{sys}:{num}:MEASCNT".format(sys=self.sys, num=self.edef_num)
        )
        self.ctrl_pv = epics.PV(
            "EDEF:{sys}:{num}:CTRL".format(sys=self.sys, num=self.edef_num)
        )
        self.num_to_acquire_pv = epics.PV(
            "EDEF:{sys}:{num}:CNTMAX".format(sys=self.sys, num=self.edef_num)
        )
        self.num_acquired_pv = epics.PV(
            "EDEF:{sys}:{num}:CNT".format(sys=self.sys, num=self.edef_num)
        )
    ...
    def reserve_edef(self, name, sys, accelerator, user=None):
        epics.caput(
            "IOC:{iocloc}:EV01:EDEFNAME".format(iocloc=self.ioc_location),
            name,
            wait=True,
        )
        timeout = 5.0
        ...
        while time_elapsed < timeout:
            for num in range(1, 16):
                edef_name = epics.caget(
                    "EDEF:{sys}:{num}:NAME".format(sys=sys, num=num)
                )
                if edef_name == name:
                    if user is not None:
                        epics.caput(
                            "EDEF:{sys}:{num}:USERNAME".format(sys=sys, num=num),
                            str(user),
                        )
                    return num
            time.sleep(0.05)
        ...
        edefs_remaining_pv = "IOC:{iocloc}:EV01:EDEFAVAIL".format(
            iocloc=self.ioc_location
        )
    ...
    def get_masks(self, mask_type):
        ...
        pv_template = "EDEF:{sys}:{num}:{mask_type}".format(
            sys=self.sys, num=self.edef_num, mask_type=mask_type
        )
        pv_template += "{n}"
    ...
    def clear_masks(self, mask_type):
        pvs = [
            "EDEF:{sys}:{num}:{mask_type}{n}".format(
                sys=self.sys, num=self.edef_num, mask_type=mask_type, n=num
            )
    ...
    def populate_bit_mask_name_cache(self):
        bit_name_pvs = [
            "PNBN:{sys}:{n}:NAME".format(sys=self.sys, n=n)
            for n in range(1, NUM_MASK_BITS + 1)
        ]
        bit_pos_pvs = [
            "PNBN:{sys}:{n}:BITP".format(sys=self.sys, n=n)
            for n in range(1, NUM_MASK_BITS + 1)
        ]
        bit_names = epics.caget_many(bit_name_pvs)
        bit_positions = epics.caget_many(bit_pos_pvs)
        self.bit_mask_name_cache = {
            bit_name: bit_position
            for bit_name, bit_position in zip(bit_names, bit_positions)
        }
        self.bit_mask_reverse_cache = {
            bit_position: bit_name
            for bit_name, bit_position in zip(bit_names, bit_positions)
        }
    ...
    def get_buffer(self, pv, suffix="HST"):
        if isinstance(pv, str):
            ...
            if self.n_measurements > 0:
                ...
                buff_dict = {
                    a_pv[:-suffix_length]: buff_dict[a_pv] for a_pv in buff_dict
                }
            return buff_dict
    ...
    def get_pulse_ids(self):
        return self.get_buffer(
            "PATT:{sys}:1:PULSEID".format(sys=self.sys), suffix="HST"
        )
    ...
    def get(self, pv):
        ...
        if isinstance(pv, str):
            ...
            pv_list = [
                "{a_pv}{num}".format(a_pv=a_pv, num=self.edef_num) for a_pv in pv
            ]
